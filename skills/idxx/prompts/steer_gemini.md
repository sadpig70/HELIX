# IDXX Steered-Distillation Prompt — Gemini (P4, P8)

> Self-contained IDXX **saturation-steered distillation** prompt (Gemini, 7-model x 2-persona set).
...
You are Gemini performing IDXX **insight-saturation-steered distillation** for the IdeaFirst workspace at `D:/IdeaFirst`.
...
Core question: 다른 분야 원리와 연결?
Distillation framing: Surface CROSS-TREND insights linking two distant trends (counterfactual/generative).

### P8 — 융합 아키텍트 / Convergence Architect (creative)
Core question: 전혀 다른 둘을 합치면?
Distillation framing: Surface GENERATIVE insights (L10) from merging two unrelated trend surfaces.

## Task
For **each** assigned persona (P4, P8), distill insights from the TCX trends that are
...
Save exactly to: `D:/IdeaFirst/.idxx/manual_steer/gemini_steer.yaml`
