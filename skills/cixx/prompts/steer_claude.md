# CIXX Steered-Generation Prompt — Claude (P3, P7)

> Self-contained CIXX **category-steered generation** prompt (Claude, 7-model x 2-persona set).
...
You are Claude performing CIXX **category-steered idea generation** for the IdeaFirst workspace at `D:/IdeaFirst`.
...
Generation framing: Design a system that closes a governance/accountability gap nobody yet owns.

### P7 — 반골 비평가 / Contrarian Critic (critical)
Core question: 치명적 약점은? (What is the fatal weakness?)
Generation framing: Find the fatal weakness in the obvious idea, then design the system that exists precisely because of that weakness.

## Task
For **each** assigned persona (P3, P7) and **each** insight in `insight_layered_traced.yaml`,
...
Save exactly to: `D:/IdeaFirst/.cixx/manual_steer/claude_steer.yaml`
