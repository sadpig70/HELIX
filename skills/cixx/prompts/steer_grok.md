# CIXX Steered-Generation Prompt — Grok (P1, P6)

> Self-contained CIXX **category-steered generation** prompt (Grok, 7-model x 2-persona set).
...
You are Grok performing CIXX **category-steered idea generation** for the IdeaFirst workspace at `D:/IdeaFirst`.
...
### P1 — 파괴적 엔지니어 / Disruptive Engineer (creative)
Core question: 기존을 완전히 뒤집으면? (What if we fully invert the status quo?)
Generation framing: Invert the dominant assumption of the insight and design the system that only makes sense after the inversion.

### P6 — 미래 사회학자 / Future Sociologist (intuitive)
Core question: 10년 후 행동 변화는? (How will behavior change in 10 years?)
Generation framing: Design for a long-horizon behavioral shift the current market has not built for.

## Task
For **each** assigned persona (P1, P6) and **each** insight in `insight_layered_traced.yaml`,
...
Save exactly to: `D:/IdeaFirst/.cixx/manual_steer/grok_steer.yaml`
...
- `steer_model.family` = `grok`; `personas` = ["P1", "P6"].
