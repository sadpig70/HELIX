...
## 2. 동시성 — single-writer 계약

actuation ledger는 로컬 append-only JSONL이며 `append_actuation_ledger`는
read-then-append다 (원자적이지 않다). **계약: 하나의 ledger 파일당 single writer.**

- pilot에서는 **참가자별 별도 ledger**를 쓴다(`docs/PILOT-PROTOCOL.md`). 참가자 내부에서
  동시 감사가 가능하면 참가자 측에서 직렬화한다.
...
- 다중 동시 writer가 실제로 필요하면(범위 밖, backlog): OS 파일 잠금이나 외부 큐로
  append를 직렬화하는 어댑터를 ledger 밖에 둔다 — 결정론 core는 잠금/타이밍을 갖지
  않으므로 core에 넣지 않는다.
...
세 항목 모두 코드 결함이 아니라 운영 계약이다: AHV는 명확 실패로 fail-closed, 동시성은
single-writer 계약 + verify 탐지, 컴플라이언스는 evidence 제공(인증 주장 없음). 실제
코드 강화가 필요한 부분(다중 writer 직렬화 어댑터)은 결정론 core 밖의 backlog로 남긴다.
