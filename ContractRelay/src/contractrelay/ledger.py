"""Append-only relay receipt ledger."""
...
def ledger_report(path):
    ...
    return {
        ...
        "problems": problems,
        "events": len(events),
        "relayed": sum(event.get("receipt", {}).get("decision") == "RELAYED" for event in events),
        "blocked": sum(event.get("receipt", {}).get("decision") == "BLOCKED" for event in events),
