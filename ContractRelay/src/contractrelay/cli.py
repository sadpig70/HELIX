"""ContractRelay command line interface."""
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="contractrelay")
    sub = parser.add_subparsers(dest="command", required=True)
    sample = sub.add_parser("sample")
    sample.add_argument("kind", nargs="?", default="relayed", choices=["relayed", "blocked", "invalid-baseline"])
    ...
    if args.command == "sample":
        ...
        return 0
    if args.command == "run":
        receipt = relay(_read_json(args.case))
        if args.ledger:
            append_receipt(args.ledger, receipt, args.recorded_at)
        _emit(receipt)
        return 2 if receipt["decision"] == "INVALID" else (1 if receipt["decision"] == "BLOCKED" else 0)
